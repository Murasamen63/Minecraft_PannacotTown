...
# リザルト処理
    ...
    ## 3位
        ...
        ### スコア表示
            execute if score $PCTW.FSBT.Temp.ResultTimer PCTW.FSBT.GameTimer matches 120 run tellraw @a ["",{"text":"  "},{"score":{"name":"@r[tag=PCTW.FSBT.Player,tag=PCTW.FSBT.Temp.Score.3rd]","objective":"PCTW.FSBT.Score"},"bold":true,"color":"dark_red"},{"text":"pt","bold":true,"color":"dark_red"}]
        ### プレイヤー表示
            #### 表示
    ...
    ## 2位
        ...
        ### スコア表示
            execute if score $PCTW.FSBT.Temp.ResultTimer PCTW.FSBT.GameTimer matches 340 run tellraw @a ["",{"text":"  "},{"score":{"name":"@r[tag=PCTW.FSBT.Player,tag=PCTW.FSBT.Temp.Score.2nd]","objective":"PCTW.FSBT.Score"},"bold":true,"color":"gray"},{"text":"pt","bold":true,"color":"gray"}]
        ### プレイヤー表示
            #### 表示
    ...
    ## 1位
        ...
        ### スコア表示
            execute if score $PCTW.FSBT.Temp.ResultTimer PCTW.FSBT.GameTimer matches 560 run tellraw @a ["",{"text":"  "},{"score":{"name":"@r[tag=PCTW.FSBT.Player,tag=PCTW.FSBT.Temp.Score.1st]","objective":"PCTW.FSBT.Score"},"bold":true,"color":"gold"},{"text":"pt","bold":true,"color":"gold"}]
        ### プレイヤー表示
            #### 発行
